...
function agregarPersona() {
  ...
  //obtenemos el elemento para mostrar un error de email
  ...
  if (nombre === "") {
   ...
   // Obtenemos el valor y le quitamos los espacios al inicio y al final
  const email = inputEmail.value.trim();

  // Validamos si el email está vacío
  if (email === "") {
    msgErrorEdad.innerHTML = "Debe ingresar un email";
    hayError = true;
  } else if (!esEmailValido(email)) { 
    msgErrorEdad.innerHTML = "El formato del email no es válido";
    hayError = true;
  } else {
    // Si no hay error, limpiamos el mensaje de error
    msgErrorEdad.innerHTML = "";
    hayError = false;
  }
